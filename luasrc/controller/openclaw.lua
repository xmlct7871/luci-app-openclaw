...
local function normalize_install_base(value)
	...
	-- v2026.6.10: 默认值改为 /root/.openclaw(与 upstream OpenClaw 在 root 用户下的默认一致)
	if raw == "" then raw = "/root/.openclaw" end
...
local function get_path_info(input)
	...
	local base = input or uci:get("openclaw", "main", "install_path") or "/root/.openclaw"
	...
	local normalized = normalize_install_base(base) or "/root/.openclaw"
	return {
		install_path = normalized,
		state_dir = normalized,
		node_base = normalized .. "/node",
		npm_lib = normalized .. "/node/lib/node_modules",
		config_file = normalized .. "/openclaw.json",
		ext_dir = normalized .. "/extensions",
...
local function is_safe_openclaw_root(value)
	...
	-- v2026.6.10: 兜底白名单(应当由 paths.lua 提供)
	return value == "/root/.openclaw"
		or value:match("^/opt/openclaw$") ~= nil
		or value:match("^/mnt/[^/]+/(.openclaw|openclaw)$") ~= nil
		or value:match("^/media/[^/]+/(.openclaw|openclaw)$") ~= nil
end

-- v2026.6.10: 删除 ensure_openclaw_user() 函数
-- 不再创建 openclaw Unix 用户,服务以 root 跑

local function find_wechat_plugin_dir(install_path)
	-- v2026.6.10: 路径扁平化,无 /data/.openclaw/ 嵌套
	local sys = require "luci.sys"
	local ext_dir = install_path .. "/extensions/openclaw-weixin"
	if nixio.fs.stat(ext_dir .. "/openclaw.plugin.json", "type") then
		return ext_dir
	end
	local npm_projects = install_path .. "/extensions/npm/projects"
...
-- ═══════════════════════════════════════════
-- v2026.6.10: install_path 直接是 state dir 自身,无 /data 包装
-- 返回 state_dir 字段(替代原来的 oc_root)
local function get_install_path()
	return get_path_info().state_dir
end
...
function action_status()
	...
	-- v2026.6.10: install_path 直接是 state dir,扁平布局,只查一个正确路径
	local oc_dirs = {
	...
	-- 读取当前活跃模型 (使用自定义安装路径)
	-- v2026.6.10: 路径扁平化,config 直接在 state_dir 根
	local config_file = install_path .. "/openclaw.json"
...
function action_uninstall()
	...
	local install_path = path_info.state_dir
	...
	sys.exec("pgrep -f 'openclaw-gateway|web-pty.js|oc-config' 2>/dev/null | xargs -r kill -9 2>/dev/null")
	-- v2026.6.10: 不再按 openclaw 用户过滤(用户不存在),改为按进程名匹配
	...
	-- v2026.6.10: 不再创建 openclaw 系统用户,无需清理 /etc/passwd
...
function action_backup()
	...
	-- v2026.6.10: install_path 直接是 state dir,无 /openclaw 嵌套层
	local install_path = get_install_path()
	local node_bin = install_path .. "/node/bin/node"
	local oc_entry = ""

	-- 查找 openclaw 入口 (使用自定义安装路径)
	-- v2026.6.10: 扁平布局,只查一个正确路径
	local search_dirs = {
		install_path .. "/node/lib/node_modules/openclaw",
	}
	...
	-- v2026.6.10: 所有 env var 直接指向 state_dir 自身,无 /data/.openclaw/ 嵌套
	local env_prefix = string.format(
		"HOME=%s OPENCLAW_HOME=%s " ..
		"OPENCLAW_STATE_DIR=%s " ..
		"OPENCLAW_CONFIG_PATH=%s/openclaw.json " ..
		"PATH=%s/node/bin:%s/node/lib/node_modules/.bin:$PATH ",
	...
	if action == "create" then
		...
		-- 完整备份可能输出到 HOME (即 backup_dir),无需移动;若 HOME 被上游改写为 backup 父目录再移回
		-- v2026.6.10: state_dir 扁平化,不再有 /data 层
		sys.exec("mv " .. install_path .. "/*-openclaw-backup.tar.gz " .. backup_dir .. "/ 2>/dev/null")
		-- 提取备份文件路径
	...
	elseif action == "restore" then
		...
		local oc_data_dir = install_path
		local config_path = install_path .. "/openclaw.json"
...
function action_wechat_install()
	...
	local install_cmd = string.format(
		...
		"'test -w %s/.npm && test -w %s/.tmp && test -w %s/.openclaw' || { echo '❌ openclaw 用户无法写入 npm cache/tmp/data 目录' >> /tmp/openclaw-wechat-install.log; echo 1 > /tmp/openclaw-wechat-install.exit; exit 0; }; " ..
		"cd %s && " ..
		"'HOME=%s OPENCLAW_HOME=%s OPENCLAW_STATE_DIR=%s/.openclaw " ..
...
function action_wechat_login()
	...
	for _, d in ipairs(search_dirs) do
        ...
        local login_cmd = string.format(
                ...
                "cd %s && " ..
                "'HOME=%s OPENCLAW_HOME=%s OPENCLAW_STATE_DIR=%s/.openclaw OPENCLAW_CONFIG_PATH=%s/.openclaw/openclaw.json " ..
...
function action_wechat_logout()
        ...
        local logout_cmd = string.format(
                "cd %s && 'HOME=%s OPENCLAW_HOME=%s OPENCLAW_STATE_DIR=%s/.openclaw OPENCLAW_CONFIG_PATH=%s/.openclaw/openclaw.json " ..
                "PATH=%s/node/bin:%s/global/bin:$PATH " ..
