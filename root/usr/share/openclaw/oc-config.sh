...
# v2026.6.10: install_path 直接就是 OpenClaw state dir
...
if command -v oc_load_paths >/dev/null 2>&1 && oc_load_paths "$OC_CONFIGURED_PATH"; then
	...
	OC_STATE_DIR="$STATE_DIR"
	OC_DATA="$STATE_DIR"
else
	OC_INSTALL_PATH="$OC_CONFIGURED_PATH"
	OC_STATE_DIR="$OC_CONFIGURED_PATH"
	OC_DATA="$OC_CONFIGURED_PATH"
	NODE_BASE="${NODE_BASE:-${OC_INSTALL_PATH}/node}"
	CONFIG_FILE="${OC_INSTALL_PATH}/openclaw.json"
fi
NODE_BASE="${NODE_BASE:-${OC_INSTALL_PATH}/node}"
NPM_LIB="${NPM_LIB:-${NODE_BASE}/lib/node_modules}"
OC_STATE_DIR="${OC_STATE_DIR:-$OC_INSTALL_PATH}"
OC_DATA="${OC_DATA:-$OC_INSTALL_PATH}"
NODE_BIN="${NODE_BASE}/bin/node"
CONFIG_FILE="${CONFIG_FILE:-${OC_INSTALL_PATH}/openclaw.json}"

export HOME="$OC_INSTALL_PATH"
...
export PATH="${NODE_BASE}/bin:${NPM_LIB}/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"

# ── 查找 openclaw 入口 ──
OC_PKG_DIR=""
for d in "${NPM_LIB}/openclaw" "${NPM_LIB}/*/openclaw" "${NODE_BASE}/lib/node_modules/openclaw"; do
	# 跳过 glob 没展开的 "*" 字面量
	case "$d" in
		*"*"*) continue ;;
	esac
...
oc_cmd() {
	if [ -n "$OC_ENTRY" ] && [ -x "$NODE_BIN" ]; then
		# v2026.6.10: 以 root 跑,不再需要 chown 修复
...
configure_feishu() {
	...
	# 执行官方安装命令(v2026.6.10: cd 到 state dir 自身,不再是 data 包装)
	cd "$OC_INSTALL_PATH"
