...
// v2026.6.10: install_path 直接是 OpenClaw state dir(类比 upstream ~/.openclaw/)
// 默认值 /root/.openclaw 与 upstream OpenClaw 在 root 用户下的默认路径完全一致
const { execSync } = require('child_process');
function normalizeInstallPath(raw) {
  let value = String(raw || '').trim().replace(/\/+$/, '');
  if (!value) value = '/root/.openclaw';
  if (!value.startsWith('/')) return '/root/.openclaw';
  if (/[\s'"`$;&|<>()]/.test(value)) return '/root/.openclaw';
  if (/^\/(proc|sys|dev|tmp|var|etc|usr|bin|sbin|lib|rom|overlay)(\/|$)/.test(value) || value === '/') return '/root/.openclaw';
  return value;
}
let installPath = '/root/.openclaw';
try {
  const uciPath = execSync('uci -q get openclaw.main.install_path 2>/dev/null', { encoding: 'utf8', timeout: 3000 }).trim();
  if (uciPath) installPath = normalizeInstallPath(uciPath);
} catch {}
// v2026.6.10: 路径扁平化,所有内容直接在 installPath 下
const NODE_BASE = process.env.NODE_BASE || installPath + '/node';
const NPM_LIB = process.env.NPM_LIB || NODE_BASE + '/lib/node_modules';
const STATE_DIR = process.env.STATE_DIR || installPath;
const CONFIG_FILE = STATE_DIR + '/openclaw.json';
...
class PtySession {
  ...
  _spawnPty() {
    const env = {
      ...
      NODE_BASE, NPM_LIB, STATE_DIR, CONFIG_FILE,
      HOME: STATE_DIR,
      OPENCLAW_HOME: STATE_DIR,
      OPENCLAW_STATE_DIR: STATE_DIR,
      OPENCLAW_CONFIG_PATH: CONFIG_FILE,
      PATH: `${NODE_BASE}/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin`,
    };
    ...
    this.proc.on('close', (code) => {
      ...
      // v2026.6.10: PTY 以 root 运行,子脚本创建的文件天然属主 root,Gateway 也是 root 跑
      // 不再需要 chown -R openclaw:openclaw OC_DATA
