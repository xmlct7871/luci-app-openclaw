...
const OC_INSTALL_PATH = process.env.OC_INSTALL_PATH || '/root/.openclaw';
const NODE_BASE = process.env.NODE_BASE || `${OC_INSTALL_PATH}/node`;
const NPM_LIB = process.env.NPM_LIB || `${NODE_BASE}/lib/node_modules`;
const OC_STATE_DIR = process.env.OPENCLAW_STATE_DIR || OC_INSTALL_PATH;
const OC_DATA = process.env.OC_DATA || OC_INSTALL_PATH;
const CONFIG_FILE = process.env.OPENCLAW_CONFIG_PATH || `${OC_INSTALL_PATH}/openclaw.json`;
...
function findOpenClawEntry() {
  ...
  try {
    ...
    const dirs = fs.readdirSync(NPM_LIB);
    for (const dir of dirs) {
      const p = `${NPM_LIB}/${dir}/openclaw/openclaw.mjs`;
      if (fs.existsSync(p)) return p;
    }
  } catch {}
  return null;
  ...
  while (true) {
    ...
    switch (choice.value) {
      ...
      case 'feishu': {
        ...
        try {
          await runCommand('npx', ['-y', '@larksuite/openclaw-lark-tools', 'install'], { env: { HOME: OC_INSTALL_PATH } });
        } catch (e) {
