...
**v2026.6.10 是重大架构重构版本**:
- 目录布局完全对齐上游 OpenClaw,默认 `install_path = /root/.openclaw`(即 upstream 的 `~/.openclaw/`)
- 不再有 `/opt/openclaw/{node,global,data}` 包装层
- 不再创建 `openclaw` Unix 用户,服务以 root 跑(OpenWrt 惯例)
- **不支持从 v2.0.6 自动升级,需要手动卸载 + 备份 + 重装** (见 [v2 → v2026.6.10 升级指南](#v2--v2026610-升级指南))
...
UCI 字段是 `openclaw.main.install_path`，v2026.6.10 语义为 **state dir 自身**(不再是"基础路径")。例如：
...
v2026.6.10 微信插件**不再依赖** `openclaw` Unix 用户(改用 root 跑)。安装前会检查:
...
**v2026.6.10 不提供数据自动迁移**。从 v2.0.6 升级需要手动操作。原因是 v2 的 `/opt/openclaw/{node,global,data}` 包装层与 v3 的扁平化布局不兼容,自动迁移容易出错。

### 步骤

```bash
# 1. 卸载 v2
opkg remove luci-app-openclaw

# 2. 备份 v2 数据(完整复制,不是 mv)
cp -a /opt/openclaw/data/.openclaw /tmp/openclaw-v2-backup
cp /etc/config/openclaw /tmp/openclaw-v2-backup-uci.conf

# 3. 清理 v2 文件(可选,但推荐,避免混乱)
rm -rf /opt/openclaw

# 4. 安装 v2026.6.10
opkg install luci-app-openclaw_2026.6.10-1_all.ipk

# 5. 第一次进 LuCI → 服务 → OpenClaw
#    看到 v2 数据警告(此时 v2 已经卸载,只显示引导信息)
#    点击「安装运行环境」

# 6. 重新配置
#    - 进入「Web 控制台」添加 AI 模型和 API Key
#    - 进入「配置管理」配置消息渠道
#    - 微信插件需要重新安装(账号信息在备份里,可手动 cp -a 回 ${install_path}/extensions/openclaw-weixin/)
```
...
```bash
# 找到 v2 备份里的微信账号文件
ls /tmp/openclaw-v2-backup/extensions/openclaw-weixin/accounts.json 2>/dev/null

# 复制到 v3 路径
cp -a /tmp/openclaw-v2-backup/extensions/openclaw-weixin/accounts.json \
      /root/.openclaw/extensions/openclaw-weixin/accounts.json
...
- v2 的 `/opt/openclaw/data/.openclaw/openclaw.json` 是 v2 格式,可能包含 v2 特有的字段。建议在 v3 重新配置,然后只手动导入必要的 API Key 等。
- v2 的 token (UCI 里的 `openclaw.main.token`) 不能直接复用,因为 v3 会生成新的。建议让 v3 重新生成。
- v2 的 model 配置建议在 v3 重新设置,因为 v3 可能用了不同的字段名。
...
**关键变化 (相对 v2.0.6)**:
- v2: `${install_path}/openclaw/{node,global,data}/...`
- v3: `${install_path}/{node,openclaw.json,workspace,extensions,...}` (扁平化)
- 不再有 `/opt/openclaw/{node,global,data}` 包装层
