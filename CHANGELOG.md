...
**这是 v2 → v3 重大重构,版本号与上游 OpenClaw 对齐。**

#### 1. 默认安装路径变更
- 旧 (v2.0.6): `${install_path}/openclaw/{node,global,data}` (默认 install_path=`/opt`,实际跑目录 = `/opt/openclaw`)
- 新 (v2026.6.10): `install_path` 直接就是 state dir 自身,默认 `/root/.openclaw`(与上游 OpenClaw 在 root 用户的默认路径一致)

#### 2. 目录布局扁平化(匹配 upstream OpenClaw)
- 不再有 `/opt/openclaw/{node,global,data}` 包装层
- Node.js 运行时和 OpenClaw 包都装在 `${install_path}/node/` 下
- `${install_path}/` 根直接展开 `openclaw.json`、`workspace/`、`extensions/`、`secrets.json`、`agents/`、`hooks/`、`logs/`、`backups/`、`npm/projects/`(与 upstream `~/.openclaw/` 平铺布局完全一致)

#### 3. 以 root 用户运行服务
- 删除 `openclaw` Unix 用户创建逻辑
- 删除所有 75 处 `chown openclaw:openclaw` 引用(跨 8 个文件)
- 删除 `procd_set_param user openclaw`(procd 默认以 root 启动 gateway)
- 微信插件安装去掉 `su openclaw -c '...'`,直接以 root 跑

#### 4. v2 风格数据不自动迁移
- v3 检测到 v2 风格数据(如 `/opt/openclaw/data/.openclaw/openclaw.json`)时拒绝启动
- 在 `/tmp/openclaw-v2-detect.log` 写明升级步骤
- 用户必须先卸载 v2,手动备份数据,再装 v3(详细步骤见 README)

#### 5. 路径白名单重写 (`is_safe_openclaw_root`)
- 接受 v3 upstream 默认 (`/root/.openclaw`)
- 接受 v2 风格包装 (`/opt/openclaw` 等,用于兼容用户改 UCI)
- 接受外置盘 (`/mnt/<x>/.openclaw`、`/mnt/<x>/openclaw` 等)
- 接受 OverlayFS 兼容路径

### OpenClaw 适配

- 适配上游 [OpenClaw v2026.6.10](https://github.com/openclaw/openclaw/releases/tag/v2026.6.10)
- 显式传 `OPENCLAW_STATE_DIR`、`OPENCLAW_CONFIG_PATH`、`HOME` 等 env var,避开 `~/.openclaw/` 解析歧义
- 新增 `NPM_CONFIG_CACHE`、`TMPDIR`、`XDG_CACHE_HOME` 隔离目录,防止污染系统其它用户
- jiti 编译缓存从 `/tmp/jiti` 改到 `${install_path}/.cache/jiti/`
- 微信插件 npm 子项目以 root 创建,`chmod 755` 保持安全策略(OpenClaw v2026.4.9+ 安全要求)

### OverlayFS 兼容性

- `_oc_fix_opt` 改名为 `_oc_fix_overlay`,同时支持 `/root` 和 `/opt` 两种 base
- 新增 fallback:`/opt` 或 `/root` 完全不可写时,使用 `/tmp/openclaw-fallback-$$` 替代

### 微信插件依赖变化

- 旧:依赖 `openclaw` Unix 用户存在(uid 1000+, shell=/bin/false)
- 新:以 root 跑,所有目录天然可写
- README 中"微信插件依赖"段落已重写

### 已知变更

- `install_path` UCI 字段语义变更:从"基础路径"改为"state dir 路径"
- `/etc/config/openclaw` 默认值从 `option install_path '/opt'` 改为 `option install_path '/root/.openclaw'`
- procd 服务环境变量:不再传 `OC_GLOBAL`、`OC_DATA`(已废弃),改传 `STATE_DIR`、`NPM_LIB`、`HOME`
